...
## 14 · 终审复确认 + R10 npm 发布任务(2026-06-28 · Opus)

**终审**:本次上传 HEAD = `b912942`(上轮 Opus M6 封签),本地未在其上新增提交,工作树干净——**状态即上轮已封态**。复确认:TS tsc 干净 + 74/0/1,15 个 conformance 测试在,封板完好。**M6 仍封板,无新工作需裁决。**

**R10 npm 发布(发布任务,非框架里程碑)**:把 `ts/` 作为 npm 包 `dopdb` 发布。Opus 已做并验证:package.json 发布化(repository+directory/keywords/publishConfig/main-module-types/prepublishOnly 等)、新增 `ts/LICENSE`(MIT)、重写 `ts/README.md`(反映当前系统,替换过时版)、新增 `.github/workflows/publish-npm.yml`。沙箱验证:`npm run build` 全 exports 入口产出;`npm pack --dry-run` 产物正确(64.6 kB,仅 dist+README+LICENSE);包名 `dopdb` 未被占用。**交本地 R10**(`rounds/R10/directive.md`):刷新 lockfile + 本机复验 + 真实 `npm publish` + app 安装冒烟 + CI secret。

## 15 · Bun 兼容封板(2026-06-30 · R11 · Opus 终判)

**✅ SEALED —— `bun run` 兼容特性成立。** 详见 `rounds/R11/SEALED.md`。

让已发布的 `@kequnyang/dopdb` 能在 bun 下运行(此前只能 npm)。根因:`bson@7`(mongodb@7 自带)的 class static 块调 `v8.startupSnapshot.isBuildingSnapshot()`,Bun 未实现 → import 期(命名绑定→链接期)崩。修复(本地编写,3 文件,零业务逻辑):server.ts 改 `import type`(擦除→无静态 mongodb import)+ 顶层 v8 shim(Bun 下把抛错 stub 换成 `()=>false`,Node 下 no-op)+ 惰性动态 `import("mongodb")`;package.json mongodb 保持 optional peer(`^6||^7`,**非 dependency** → client 不拉)+ version `0.1.202606301409`;lockfile 重生(resolved 回标准 registry)。

**Opus 独立验证**(沙箱 node v22 + **亲装 bun 1.3.14**):npm ci 一致;tsc+build 过;编译产物零静态 mongodb import、shim 在顶层、动态 import 在;**全测 74/0/1**(补上本地 Node19 跑不了的 CI 路径);Node e2e 过;client/index 零 mongodb;**bun 对照(不经 shim 直接 import mongodb 真崩 NotImplementedError)+ bun 修复(server.js import + 动态 mongodb + new MongoClient 通过)亲跑确认**;npm pack 产物正确。

非阻塞跟进:① ~~bun + 真副本集下真实 DB 操作未验~~ **[2026-06-30 补验闭合]** —— 本地 GLM-5.2 在 bun 1.3.14 + 独立 mongod 8.3.4 单节点副本集 `rs0` 上跑通:hset 真写 200 / hget 真读回 `{"owner":"alice"}`(owner-scope 生效)/ watch 建流 200 text/event-stream / 触发后 raw socket 抓到 SSE 事件(`DATA len 351` 含 resume token)。import 期病灶之外的真实 DB 操作路径已确证。详见 `rounds/R11/SEALED.md` 补验节。② `serverDb(schema,db)` 进阶路径若消费者自行静态 import mongodb 仍需自管(主用法 `serve({mongo})` 不受影响),可选把 shim 作 `@kequnyang/dopdb/bun` 子入口导出。

**版本 bump(供 npm 推送)**:`0.1.202606301409` → **`0.1.202606301552`**(ts/package.json + ts/package-lock.json 三处版本号一致;lockfile `up to date`)。GH Action(`publish-npm.yml`)触发即推 `@kequnyang/dopdb@0.1.202606301552`。

**项目状态**:M0–M5+HttpOn(R8)、M6 redisdb-compat(R9)、npm 发布(R10,`@kequnyang/dopdb` 已上线)、bun 兼容(R11)均已 Opus 终判。
