...
// a live server; this file locks the one guard that needs no database.
...
// startup, BEFORE opening any connection, so this test needs no server and is fast.
test("serve() rejects a scoped collection whose owner field is unbound (fail-closed)", async () => {
  ...
  await assert.rejects(
    serve({ schema, kvrocks: { uri: "redis://127.0.0.1:1", namespace: "x" }, jwtSecret: "s" }),
